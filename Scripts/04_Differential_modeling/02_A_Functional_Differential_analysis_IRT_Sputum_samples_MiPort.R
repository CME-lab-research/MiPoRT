...
# Mirrors the condensed IRT species workflow, but uses KO feature tables instead.
# Main idea:
#   1. Read KO feature table + metadata
#   2. Keep IRT samples only
...
# KO feature table
KO_TABLE_PATH <- "../01_KO_abundance_filtering/IRT_featureTable_KO_filt_AbundMeanAtLeastOneDisease_1eminus04.tsv"
...
# ------------------------------------------------------------------
# 3. Read metadata and keep IRT only
# ------------------------------------------------------------------
...
# sanity check
table(Metadata_Df$Disease, useNA = 'ifany')
...
print(table(Metadata_Df_DOI_sub$Dataset_Group, 
...
for (each_Grp in Unique_Runs) {
    ...
    KO_profiles_sub <- KO_profiles_DOI %>%
...
# ------------------------------------------------------------------
# 8. Run MaAsLin3 for all vars in 1 model
# ------------------------------------------------------------------
...
# 9. End
# ------------------------------------------------------------------
message("Done running KO-based IRT MaAsLin3 models.")
