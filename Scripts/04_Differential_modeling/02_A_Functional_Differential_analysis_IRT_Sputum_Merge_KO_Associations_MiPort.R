...
# For IRT 
#    take: 'significant_results.tsv' from the "IRT_KO_Model-1" 
#    read and join (rowbind): all_results.tsv from each RT_KO_CF-Dataset-{N}_Model-2
#    output: "IRT_Maaslin3_merged_Coeff.txt"

# Load env
library(tidyverse)

# set wd
setwd("C:/Users/o_shinde/Desktop/MiPORT_Functions_DA/03_Humann/03_G_Maaslin3/IRT_KO_Models")

# List of IRT datasets-Models 
Model_1_Name <- "IRT_KO_Model-1"
Model_2_Names <- list.files()[startsWith(list.files(), "IRT_KO_CF")]
...
for (eachM2 in Model_2_Names) {
    ...
    M2_Dataset_ID <- str_extract(eachM2, "CF-Dataset-[0-9]+")
...
write.table(
    Merged_df,
    file = "IRT_Maaslin3_merged_Coeff.txt",
    sep = "\t",
    quote = FALSE,
    row.names = FALSE
)
...
print("IRT_Maaslin3_merged_Coeff.txt")
